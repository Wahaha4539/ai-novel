...
向用户说明代码、文件、函数、数据模型或调用链时，必须同时给出自然语言描述，不能只堆文件名、符号名、行号或英文术语。每个关键代码引用都应说明它“负责什么”“为什么相关”“对当前问题有什么影响”，让不熟悉代码的人也能跟上讨论。

引用具体文件或代码位置时，应采用“代码引用 + 白话解释”的写法。例如：`QualityReport` 是用于记录 AI 审稿结果的数据模型，`ai-quality-review.service.ts` 是发起或汇总审稿流程的服务，`chapter-outline-quality-review.ts` 是检查章节细纲质量的逻辑。不要只写“`QualityReport` 在 schema，入口在某文件，脚本在某文件”，除非后面紧接着解释这些代码共同完成了什么。

当用户表示“不清楚代码是干嘛的”“影响沟通”或类似反馈时，应主动放慢解释节奏，先用业务语言概括，再补充必要的技术细节；必要时用一两句话说明调用顺序、输入输出和风险点。
...
涉及用户自然语言目标的数字含义时，例如“第 N 章”是目标章节号，还是“N 章细纲 / 拆成 N 章 / 改成 N 章 / 共 N 章”里的目标 `chapterCount`，不得新增程序正则、关键词窗口或启发式解析来替 LLM 判定意图。应由 LLM Planner 根据上下文输出结构化字段（如 `chapterNo`、`chapterCount`、`volumeNo` 和具体工具参数）；后端只校验这些结构化字段内部一致、引用合法、范围覆盖完整。上下文里的 `Volume.chapterCount` 只能作为用户未明确目标章数时的保守 fallback，不能压过 LLM 计划中明确给出的 `chapterCount`。
