...
| 2026-05-08 | Final reviewer persisted-volume fix | 修复 reviewer 复审发现的 guided 章节主流程阻断：`loadGuidedVolumeCharacterPlans` 读取已保存 `Volume` 时补齐 `title/synopsis/objective`，避免完整卷纲已入库但章节细纲校验因 select 漏字段失败；新增回归测试模拟 Prisma 只返回 select 字段，确认没有 session 草稿时 `guided_chapter` 可使用 persisted `Volume.narrativePlan` 完成角色执行校验并写入章节。 | `AGENT_TEST_FILTER='VCC guided_chapter resolves persisted volume narrativePlan without session draft' pnpm --filter api test:agent` 通过 1/318 项；最终 `pnpm --filter api test:agent` 通过 318/318 项，`pnpm --filter api build` 通过，`pnpm --filter web build` 通过。 |
| 2026-05-08 | Final reviewer guided chapter persist guard | 修复 reviewer 终审发现的章节写入结果失真：`guided_chapter` 按卷保存时目标 `Volume` 未持久化直接失败，不再静默返回 `Chapter × N`；全量保存时章节引用未持久化卷号也直接失败，禁止写入 `volumeId=null` 的章节；补充按卷和全量两条回归，断言失败时不触碰章节写入、不更新 guided session、不清理缓存。 | `AGENT_TEST_FILTER='VCC guided_chapter rejects per-volume write before volume is persisted' pnpm --filter api test:agent` 通过 1/320 项；`AGENT_TEST_FILTER='VCC guided_chapter rejects full write with unresolved volume references' pnpm --filter api test:agent` 通过 1/320 项；最终 `pnpm --filter api test:agent` 通过 320/320 项，`pnpm --filter api build` 通过，`pnpm --filter web build` 通过。 |
| 2026-05-08 | Final reviewer closure fixes | 修复最终 reviewer 新发现的风险：`guided_chapter` 写入前只使用已持久化 `Volume.narrativePlan.characterPlan` 校验，session 草稿不能覆盖同卷号 DB 卷纲；旧 `generate_outline_preview` 多批合并后用最终保留的卷级 `characterPlan` 重校验全部章节 `characterExecution`；`validate_guided_step_preview` 将 `supportingCharacters` 标记为 `session_only`，不再暗示会创建正式角色；同步补齐旧 supporting 测试的已持久化卷前提和缺失卷错误提示。 | `AGENT_TEST_FILTER='VCC guided_chapter rejects unpersisted session characterPlan when writing chapters' pnpm --filter api test:agent` 通过 1/321 项；`AGENT_TEST_FILTER='VCC generate_outline_preview rejects batched chapters that reference batch-only candidates' pnpm --filter api test:agent` 通过 1/322 项；`AGENT_TEST_FILTER='VCC validate_guided_step_preview marks guided_chapter supportingCharacters as session only' pnpm --filter api test:agent` 通过 1/323 项；`AGENT_TEST_FILTER='VCC guided_chapter rejects per-volume write before volume is persisted' pnpm --filter api test:agent` 通过 1/323 项；`AGENT_TEST_FILTER='VCC guided_chapter finalize does not create supporting Character' pnpm --filter api test:agent` 通过 1/323 项；复审无高/中风险；最终 `pnpm --filter api test:agent` 通过 323/323 项，`pnpm --filter api build` 通过，`pnpm --filter web build` 通过。 |
