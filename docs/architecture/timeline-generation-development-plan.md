...
- TL-P2-02：新增只读 `validate_timeline_preview` 工具，重新 normalize preview、核对 sourceTrace/章节引用/重复事件，输出 accepted、rejected 与 TimelineEvent writePreview，并测试 rejected 候选和伪造 agentRunId 直接失败；涉及文件：`apps/api/src/modules/agent-tools/tools/validate-timeline-preview.tool.ts`、`apps/api/src/modules/agent-runs/agent-services.spec.ts`、`docs/architecture/timeline-generation-development-plan.md`；验证命令：`pnpm --filter api test:agent`、`pnpm --filter api build`、`git diff --check`。
...
- TL-P5-02：新增 `TimelineUpdatePreview` 组件，按 create_planned/confirm_planned/update_event/archive_event/create_discovered/reject 展示时间线写入前 diff、前后快照、字段变化和 sourceTrace，并接入 timeline validation artifact；涉及文件：`apps/web/components/agent/TimelineUpdatePreview.tsx`、`apps/web/components/agent/AgentArtifactPanel.tsx`、`docs/architecture/timeline-generation-development-plan.md`；验证命令：`pnpm --filter web build`、`git diff --check`。
...
| TL-P1-05 | done | 测试 LLM 失败、JSON 不完整、字段缺失、数量不足直接抛错 | `agent-services.spec.ts` | 不产生可审批候选，不写库 |
...
| TL-P2-02 | done | 新增 `validate_timeline_preview` 工具 | `apps/api/src/modules/agent-tools/tools/validate-timeline-preview.tool.ts` | 输出 accepted/rejected/writePreview |
...
| TL-P4-06 | done | 写入后清理召回缓存 | persist 工具、service | 成功写 `TimelineEvent` 后调用 `NovelCacheService.deleteProjectRecallResults(projectId)` |
...
| TL-P5-02 | done | 新增时间线更新预览组件 | `apps/web/components/*` | 展示 create/update/archive/confirm diff |
| TL-P5-03 | done | Generation Config 文案更新 | `GenerationConfigPanel.tsx` | `autoUpdateTimeline` 明确说明会运行校验和写入策略 |
| TL-P5-04 | done | Agent Artifact 支持 timeline-only preview/validate/persist 结果 | `AgentArtifactPanel.tsx` | 与 continuity mixed diff 并存 |
| TL-P5-05 | done | 浏览器真实测试 | Docker Compose + Browser Use/Playwright | 时间线页面能看到生成后的状态变化 |
