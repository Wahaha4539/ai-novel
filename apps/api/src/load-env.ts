...
/**
 * Loads the repository-level environment file before other modules read process.env.
 *
 * Inputs: none.
 * Outputs: dotenv result is intentionally ignored because Nest/Prisma will report
 * missing required values during startup.
 * Side effects: populates process.env from the root .env without overriding values
