...
润色重点：
0. 润色前先进行内部评分：按 0-10 分评估事实保真、AI味、画面感/感官细节、节奏衔接、对话自然度、角色语气一致性；评分只用于改写决策，严禁输出。
1. 优先修复内部评分低于 8 分的维度，尤其是事实保真、AI味和节奏问题。
2. 删除或替换常见 AI 腔、总结腔和空泛形容。
3. 用动作、感官细节和节奏变化替代直接情绪说明。
