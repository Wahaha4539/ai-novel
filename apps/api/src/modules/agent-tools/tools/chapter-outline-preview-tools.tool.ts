...
export class GenerateChapterOutlinePreviewTool implements BaseTool<GenerateChapterOutlinePreviewInput, ChapterOutlinePreviewOutput> {
  ...
  private buildSystemPrompt(): string {
    return [
      ...
      '生成前先在内部完成章节设计检查，但不要把检查项作为额外字段输出；所有结果必须落入 objective、conflict、hook、outline 和 craftBrief 现有字段。',
      '章节设计检查包含：主要事件链、主角表层目标与深层驱动力、外部/关系/认知阻碍、冲突升级、读者期待点、伏笔或情绪锚点、章末钩子。',
      'outline、actionBeats、sceneBeats 必须呈现递进链：开场处境或目标 -> 触发冲突 -> 阻力升级或权威/代价显形 -> 转折/发现/失控 -> 不可逆结果和下一章压力；可按题材调整，但不能是并列流水账。',
      'visibleGoal 写主角眼前可执行目标；hiddenEmotion 写更深层恐惧、渴望、自我怀疑或认知裂缝；coreConflict 和 sceneBeats.obstacle 写清阻碍如何逼迫选择。',
      ...
      '章节细纲不得用自然语言新增重要角色；若角色来自新增候选，characterExecution.cast[].source 必须是 volume_candidate，characterName 必须等于上游 newCharacterCandidates[].name。',
      'craftBrief.actionBeats 至少 3 个节点；sceneBeats 至少 3 个场景段；concreteClues 至少 1 个且包含 name、sensoryDetail、laterUse。',
      'concreteClues 不限物证，也可以是小道具、异常行为、环境细节、台词误导或情绪锚点；laterUse 必须说明它后续如何被回收、反转或加压。',
      ...
      'hook、handoffToNextChapter 和 continuityState.nextImmediatePressure 必须制造具体翻页问题：新危险、未付代价、信息反转、关系裂痕或世界规则变化，禁止泛写“留下悬念”。',
  ...
  private buildUserPrompt(args: GenerateChapterOutlinePreviewInput, volumeNo: number, chapterNo: number, chapterCount: number, previousUnitChapters: unknown[] = []): string {
    ...
    return [
      ...
      '结构化返回要求：不要在 outline、risks 或说明文字里临时新增重要角色；重要新增角色只能由上游卷纲的 newCharacterCandidates[] 提供，本章只能用 source=volume_candidate 引用。',
      '',
      '设定摘要：',
      this.safeJson(Array.isArray(context.lorebookEntries) ? context.lorebookEntries.slice(0, 30) : [], 4000),
      '',
      '本章创作维度（请内化到现有字段，不要新增字段或输出分析报告）：',
      '- 写清主要事件链：本章从什么处境起步，因什么冲突被推动，最后留下什么不可逆状态。',
      '- 写清主角目标分层：visibleGoal 是眼前动作目标，hiddenEmotion/characterShift 是更深层的恐惧、欲望、自我怀疑或认知变化。',
      '- 写清冲突升级：至少让一个压力从小到大升级，并在 sceneBeats.obstacle、turningPoint、partResult 中落到可见动作和结果。',
      '- 写清读者期待点与伏笔锚点：hook、concreteClues、laterUse 要服务后续回收、反转、情绪刺点或世界观问题。',
      '',
