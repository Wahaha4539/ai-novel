...
export class PolishChapterTool implements BaseTool<PolishChapterInput, PolishChapterResult> {
  ...
  manifest: ToolManifestV2 = {
    ...
    whenToUse: ['用户要求润色、去 AI 味、去除机器感、增强压迫感或修改当前章节', '用户要求改稿但不覆盖正式正文', '需要使用 humanizer-polish 流程做表达层减法和最后一遍 anti-AI pass'],
    whenNotToUse: ['用户只是检查问题不要求修改', '没有真实 chapterId 且未调用 resolve_chapter', '章节没有可用草稿且用户要求写新正文时应使用 write_chapter'],
    inputSchema: this.inputSchema,
    outputSchema: this.outputSchema,
    parameterHints: { chapterId: { source: 'resolver', resolverTool: 'resolve_chapter', description: '真实章节 ID。' }, draftId: { source: 'context', description: '当前草稿 ID，可来自 context.session.currentDraftId 或 runtime.currentDraftId。' }, instruction: { source: 'user_message', description: '必须保留用户禁改项，例如“别改结局”；若用户要求去 AI 味，应原样保留该意图以触发 humanizer-polish 流程。' } },
