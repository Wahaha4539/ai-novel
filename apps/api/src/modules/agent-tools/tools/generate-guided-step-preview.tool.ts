...
const SUPPORTED_STEP_INSTRUCTIONS: Record<GuidedStepSchemaKey, string> = {
  ...
  guided_chapter: '为指定卷生成章节细纲和本卷配角；每章必须有主要事件链、主角表层目标与深层驱动力、具体阻碍、冲突升级、读者期待点、伏笔或情绪锚点、章末钩子，并落入所属 storyUnit、3-5 个连续场景段、跨章交接、滚动连续状态和完整 craftBrief。若传入 chapterNo，则只细化该单章。',
        ...
        },
        {
          ...
          content: [
            ...
            stepKey === 'guided_chapter'
              ? '章节连续性硬要求：章节不是场景边界，而是阅读节奏边界；每章必须内化主要事件链、主角表层目标与深层驱动力、具体阻碍、冲突升级、读者期待点、伏笔或情绪锚点、章末钩子，并写入 objective/conflict/hook/outline/craftBrief，不新增字段；每 3-5 章必须组成完整 storyUnit 单元故事；每章 craftBrief.storyUnit 必须写清 unitId、chapterRange、chapterRole、至少 3 项 serviceFunctions，以及主线/人物/关系/世界主题贡献。sceneBeats 至少 3 个场景段，跨章节场景用同一 sceneArcId 串联；每章必须包含 entryState、exitState、openLoops、closedLoops、handoffToNextChapter 和 continuityState。禁止只写推进/建立/完成等抽象目标。'
              : '',
