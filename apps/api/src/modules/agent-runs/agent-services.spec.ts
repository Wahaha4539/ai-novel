...
test('generate_story_units_preview 不从 instruction 或卷目标文本推导目标章数', async () => {
  let calls = 0;
  const tool = new GenerateStoryUnitsPreviewTool({
    async chatJson() {
      calls += 1;
      return {
        data: { volumeNo: 1, chapterCount: 45, storyUnitPlan: createVccStoryUnitPlan(45), risks: [] },
        result: { model: 'mock-story-units-should-not-run' },
      };
    },
  } as never);
    ...
    () => tool.run(
      {
        ...
        instruction: '把第一卷重拆成45章并生成单元故事',
        volumeNo: 1,
      },
      { agentRunId: 'run-story-units-no-text-infer', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
    ),
    /requires structured chapterCount/,
  );
  assert.equal(calls, 0);
});

test('generate_story_units_preview 拒绝 args.chapterCount 与 volumeOutline.chapterCount 不一致', async () => {
  const tool = new GenerateStoryUnitsPreviewTool({
    async chatJson() {
      return {
        data: { volumeNo: 1, chapterCount: 45, storyUnitPlan: createVccStoryUnitPlan(45), risks: [] },
        result: { model: 'mock-story-units-mismatch' },
      };
    },
  } as never);
  ...
  await assert.rejects(
    () => tool.run(
      {
        volumeOutline: { volumeNo: 1, title: '旧卷纲', chapterCount: 60, narrativePlan: createVccNarrativePlanForChapterCount(60, { storyUnits: undefined }) },
        instruction: '重拆成 45 章',
        volumeNo: 1,
        chapterCount: 45,
      },
      { agentRunId: 'run-story-units-count-mismatch', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
    ),
    /chapterCount 45.*volumeOutline\.chapterCount 60/,
  );
});
...
test('PolishChapterService 润色提示接入 humanizer anti-AI 流程', () => {
  ...
  assert.match(HUMANIZER_POLISH_GUIDE, /低信息短句/);
  assert.match(HUMANIZER_POLISH_GUIDE, /没人回头看他/);
  assert.match(HUMANIZER_POLISH_GUIDE, /押兵没出声/);
  assert.match(HUMANIZER_POLISH_GUIDE, /无叙事功能细节/);
  ...
  assert.match(prompt, /他没有回答/);
  assert.match(prompt, /没出声、没反应、又看了一眼/);
  assert.match(prompt, /不改变判断、阻力、危险、关系或线索/);
...
test('persist_volume_outline maps volume-local foreshadow ranges to global chapter numbers', async () => {
  const createdForeshadowTracks: Array<Record<string, unknown>> = [];
  ...
  const prisma = {
    ...
    chapter: {
      ...
      async aggregate(args: unknown) {
        ...
        return { _max: { chapterNo: 120 } };
      },
    },
    volume: {
      async findUnique() { return null; },
      async findMany() { return [{ id: 'v1' }, { id: 'v2' }]; },
      async upsert() { return { id: 'v3' }; },
    },
    async $transaction(callback: (tx: Record<string, unknown>) => Promise<unknown>) {
      return callback({
  ...
  const preview = {
    volume: {
      ...
      narrativePlan: createVccNarrativePlanForChapterCount(3, {
        foreshadowPlan: [{
          name: 'local range clue',
          appearRange: { start: 1, end: 1 },
          recoverRange: { start: 3, end: 3 },
          recoveryMethod: 'pay off in local chapter 3',
        }],
      }),
    },
    risks: [],
  ...
  await tool.run(
    { preview },
    { agentRunId: 'run-persist-volume-outline-global-ranges', projectId: 'p1', mode: 'act', approved: true, outputs: {}, policy: {} },
  );

  assert.equal(aggregateCalls.length, 1);
  assert.equal(createdForeshadowTracks[0].chapterNo, 121);
  assert.equal(createdForeshadowTracks[0].firstSeenChapterNo, 121);
  assert.equal(createdForeshadowTracks[0].lastSeenChapterNo, 123);
});
...
test('PersistOutlineTool 写入新建和 planned 章节 craftBrief 并跳过 drafted', async () => {
  ...
  preview.chapters[1].craftBrief = createOutlineCraftBrief({
    visibleGoal: (preview.chapters[1].craftBrief as Record<string, unknown>).visibleGoal,
    concreteClues: [{ name: 'local payoff clue', sensoryDetail: 'visible detail', laterUse: 'same chapter payoff', scope: 'chapter' }],
  });
  const result = await tool.run(
    { preview },
    { agentRunId: 'run1', projectId: 'p1', mode: 'act', approved: true, outputs: {}, policy: {} },
  ...
  const chapterOutlineTracks = createdForeshadowTracks.filter((track) => track.source === 'chapter_outline');
  const crossChapterTrack = chapterOutlineTracks.find((track) => track.title !== 'local payoff clue');
  assert.equal(crossChapterTrack?.scope, 'cross_chapter');
  assert.equal(crossChapterTrack?.lastSeenChapterNo, undefined);
  const chapterLocalTrack = chapterOutlineTracks.find((track) => track.title === 'local payoff clue');
  assert.equal(chapterLocalTrack?.scope, 'chapter');
  assert.equal(chapterLocalTrack?.lastSeenChapterNo, chapterLocalTrack?.firstSeenChapterNo);
...
test('GenerateOutlinePreviewTool keeps long outer timeout without output token cap', async () => {
  ...
  const output = await tool.run(
    ...
    },
    {
      ...
      approved: false,
      outputs: {
        2: { chapter: createOutlineChapter(1, 1, { title: 'UNIT_PREVIOUS_CHAPTER_ONE', hook: 'UNIT_PREVIOUS_HOOK_ONE', outline: 'UNIT_PREVIOUS_OUTLINE_ONE' }) },
      },
      stepTools: { 2: 'generate_chapter_outline_preview' },
      policy: {},
...
test('Planner prompt 将长章节细纲引导到 outline_design 而非正文写作', async () => {
  ...
  const llm = {
    async chatJson(messages: Array<{ role: string; content: string }>, options: { timeoutMs?: number }) {
      ...
      const chapterSteps = Array.from({ length: 60 }, (_, index) => {
        const chapterNo = index + 1;
        const stepNo = index + 2;
        return {
          stepNo,
          name: `生成第 ${chapterNo} 章细纲`,
          tool: 'generate_chapter_outline_preview',
          mode: 'act',
          requiresApproval: false,
          args: {
            ...
            chapterCount: 60,
            chapterNo,
            ...(index > 0 ? { previousChapter: `{{steps.${stepNo - 1}.output.chapter}}` } : {}),
          },
        };
      });
      return {
        data: {
          ...
          steps: [
            ...
            ...chapterSteps,
            { stepNo: 62, name: '合并 60 章单章细纲', tool: 'merge_chapter_outline_previews', mode: 'act', requiresApproval: false, args: { previews: chapterSteps.map((step) => `{{steps.${step.stepNo}.output}}`), volumeNo: 1, chapterCount: 60 } },
            { stepNo: 63, name: '审批后写入细纲', tool: 'persist_outline', mode: 'act', requiresApproval: true, args: { preview: '{{steps.62.output}}' } },
          ],
        },
        result: { model: 'planner-mock' },
      };
  ...
  const plan = await planner.createPlan('帮我重写第一卷的章节细纲');
  const promptPayload = JSON.parse(capturedMessages[1].content);
  ...
  assert.equal(plan.steps.some((step) => step.tool === 'generate_chapter_outline_batch_preview'), false);
  assert.deepEqual(plan.steps.slice(0, 4).map((step) => step.tool), ['inspect_project_context', 'generate_chapter_outline_preview', 'generate_chapter_outline_preview', 'generate_chapter_outline_preview']);
  assert.equal(plan.steps[1].args.chapterCount, 60);
  assert.equal(plan.steps[1].args.chapterNo, 1);
  assert.equal(plan.steps[60].args.chapterNo, 60);
  ...
  assert.equal(plan.steps[62].tool, 'persist_outline');
  assert.deepEqual(plan.steps[62].args, { preview: '{{steps.62.output}}' });
  assert.equal(plan.steps[62].requiresApproval, true);
  assert.equal(capturedOptions?.timeoutMs, DEFAULT_LLM_TIMEOUT_MS);
  assert.match(capturedMessages[0].content, /章节细纲 \/ 第 N 卷章节细纲/);
  assert.match(capturedMessages[0].content, /重写第一卷的章节细纲/);
  assert.match(capturedMessages[0].content, /replace chapter outlines only/);
  assert.match(capturedMessages[0].content, /CURRENT chapter-outline rule/);
  assert.match(capturedMessages[0].content, /previousUnitChapters/);
  assert.match(capturedMessages[0].content, /不要误判为 write_chapter/);
  assert.match(promptPayload.taskTypeGuidance.outline_design, /重写第一卷的章节细纲/);
  assert.match(promptPayload.taskTypeGuidance.outline_design, /不要重新生成卷大纲或单元故事/);
  assert.match(promptPayload.taskTypeGuidance.outline_chapter_batching, /one visible generate_chapter_outline_preview step per chapter/);
  assert.match(promptPayload.taskTypeGuidance.outline_chapter_batching, /ToolContext\.outputs/);
  ...
  assert.doesNotMatch(JSON.stringify(promptPayload.availableTools), /generate_chapter_outline_batch_preview/);
});
...
test('PlanValidatorService rejects volume character persistence without explicit selection', () => {
  const validator = new PlanValidatorService();
  assert.throws(
    () => validator.validate({
      plan: createPlanValidatorPlan([{ tool: 'persist_volume_character_candidates', requiresApproval: true, args: { preview: '{{steps.4.output}}' } }]),
    }),
    /persist_volume_character_candidates without explicit/,
  );
  assert.doesNotThrow(
    () => validator.validate({
      plan: createPlanValidatorPlan([{ tool: 'persist_volume_character_candidates', requiresApproval: true, args: { preview: '{{steps.4.output}}', approveAll: true } }]),
    }),
  );
});
...
test('ASP-P6-002 selectToolBundleNode refines outline route through outline subgraph', async () => {
  ...
  assert.equal(result.route?.chapterCount, undefined);
  assert.deepEqual(result.route?.routeHints, { contextChapterCount: 30, chapterCountSource: 'context_volume' });
...
test('FactExtractorService aligns existing foreshadow lifecycle after chapter generation', async () => {
  const foreshadowUpdates: Array<{ where: Record<string, unknown>; data: Record<string, unknown> }> = [];
  const createdForeshadows: unknown[] = [];
  ...
  const existingForeshadow = {
    ...
    chapterNo: 12,
    title: '旧玉佩异动',
    detail: '旧玉佩在本章发光，并揭示为血契钥匙。',
    status: 'planned',
    scope: 'chapter',
    source: 'chapter_outline',
    firstSeenChapterNo: 12,
    lastSeenChapterNo: 12,
    metadata: { sourceKind: 'chapter_outline' },
  };
  const prisma = {
    chapter: {
      async findFirst() {
        return { id: 'c1', projectId: 'p1', chapterNo: 12, title: '雨夜', objective: '揭开玉佩作用', conflict: '师徒对峙', timelineSeq: 12, project: { title: '测试书', generationProfile: { allowNewCharacters: true, allowNewLocations: true, allowNewForeshadows: true, preGenerationChecks: [] } } };
      },
    },
    chapterDraft: { async findFirst() { return { id: 'draft1', chapterId: 'c1', content: '林烬看见旧玉佩发光，血线映出师门密印，终于明白它就是开启旧案血契的钥匙。' }; } },
    foreshadowTrack: { async findMany() { return [existingForeshadow]; } },
    async $transaction(callback: (tx: unknown) => Promise<unknown>) {
      return callback({
  ...
  const llm = {
    async chat() { return { text: '旧玉佩发光，揭开血契钥匙真相。' }; },
    async chatJson(_messages: unknown, options: { appStep: string }) {
      if (options.appStep === 'fact_extractor.events') return { data: [] };
      if (options.appStep === 'fact_extractor.states') return { data: [] };
      if (options.appStep === 'fact_extractor.foreshadows') return { data: [{ title: '旧玉佩异动', detail: '旧玉佩发光并揭示真相', status: 'resolved', scope: 'chapter' }] };
      if (options.appStep === 'fact_extractor.foreshadow_alignment') return { data: [{ foreshadowTrackId: 'foreshadow-1', status: 'resolved', evidence: '旧玉佩发光，血线映出师门密印，揭示它是血契钥匙。' }] };
      if (options.appStep === 'fact_extractor.first_appearances') return { data: [] };
      if (options.appStep === 'fact_extractor.relationships') return { data: [] };
      return { data: [] };
    },
  };
  const memoryWriter = {
    async replaceGeneratedChapterFactMemories(input: Record<string, unknown>) {
      ...
      return {
        deletedCount: 0,
        createdCount: 1,
        embeddingAttachedCount: 1,
        chunks: [{ id: 'm-summary', memoryType: 'summary', summary: '摘要', status: 'auto' }],
      };
    },
  };
  ...
  assert.equal(result.updatedForeshadows, 1);
  assert.equal(result.createdForeshadows, 0);
  assert.equal(createdForeshadows.length, 0);
  assert.equal((memoryInputs[0].foreshadows as unknown[]).length, 0);
  assert.equal(foreshadowUpdates.length, 1);
  assert.deepEqual(foreshadowUpdates[0].where, { projectId: 'p1', id: 'foreshadow-1' });
  assert.equal(foreshadowUpdates[0].data.status, 'resolved');
  assert.equal(foreshadowUpdates[0].data.lastSeenChapterNo, 12);
  assert.equal(((foreshadowUpdates[0].data.metadata as Record<string, unknown>).lastLifecycleUpdate as Record<string, unknown>).status, 'resolved');
});

test('FactExtractorService keeps future foreshadow recovery window while planting', async () => {
  const foreshadowUpdates: Array<{ where: Record<string, unknown>; data: Record<string, unknown> }> = [];
  const existingForeshadow = {
    id: 'foreshadow-1',
    projectId: 'p1',
    ...
    chapterNo: 12,
    title: 'tower tilt',
    detail: 'set up in chapter 12, recover in chapter 15',
    status: 'planned',
    scope: 'volume',
    source: 'volume_outline',
    firstSeenChapterNo: 12,
    lastSeenChapterNo: 15,
    metadata: { sourceKind: 'volume_outline' },
  };
  const prisma = {
    chapter: {
      async findFirst() {
        return { id: 'c1', projectId: 'p1', chapterNo: 12, title: 'chapter 12', objective: 'plant clue', conflict: 'pressure', timelineSeq: 12, project: { title: 'Test Novel', generationProfile: { allowNewCharacters: true, allowNewLocations: true, allowNewForeshadows: true, preGenerationChecks: [] } } };
      },
    },
    chapterDraft: { async findFirst() { return { id: 'draft1', chapterId: 'c1', content: 'The old tower tilts by one mark, but nobody knows why yet.' }; } },
    foreshadowTrack: { async findMany() { return [existingForeshadow]; } },
    async $transaction(callback: (tx: unknown) => Promise<unknown>) {
      return callback({
        storyEvent: { async deleteMany() { return { count: 0 }; }, async createMany(args: { data: unknown[] }) { return { count: args.data.length }; } },
  ...
  const llm = {
    async chat() { return { text: 'summary' }; },
    async chatJson(_messages: unknown, options: { appStep: string }) {
      if (options.appStep === 'fact_extractor.foreshadow_alignment') return { data: [{ foreshadowTrackId: 'foreshadow-1', status: 'planted', evidence: 'The old tower tilts by one mark.' }] };
      return { data: [] };
    },
  };
  const memoryWriter = {
    async replaceGeneratedChapterFactMemories() {
      return {
        deletedCount: 0,
        createdCount: 0,
        embeddingAttachedCount: 0,
        chunks: [],
      };
    },
  };
  ...
  assert.equal(result.updatedForeshadows, 1);
  assert.equal(foreshadowUpdates.length, 1);
  assert.equal(foreshadowUpdates[0].data.status, 'planted');
  assert.equal(Object.prototype.hasOwnProperty.call(foreshadowUpdates[0].data, 'lastSeenChapterNo'), false);
  assert.equal(((foreshadowUpdates[0].data.metadata as Record<string, unknown>).lastLifecycleUpdate as Record<string, unknown>).chapterNo, 12);
});

test('FactExtractorService rejects foreshadow lifecycle alignment when LLM omits candidates', async () => {
  let transactionCalled = false;
  const existingForeshadow = {
    id: 'foreshadow-1',
    projectId: 'p1',
  ...
  const prisma = {
    ...
    chapterDraft: { async findFirst() { return { id: 'draft1', chapterId: 'c1', content: '旧玉佩发光，血线映出师门密印。' }; } },
    foreshadowTrack: { async findMany() { return [existingForeshadow]; } },
    async $transaction() {
      transactionCalled = true;
      return {};
  ...
  const llm = {
    async chat() { return { text: '旧玉佩发光，血线映出师门密印。' }; },
    async chatJson(_messages: unknown, options: { appStep: string }) {
      if (options.appStep === 'fact_extractor.foreshadow_alignment') return { data: [] };
      return { data: [] };
    },
  };
  const memoryWriter = {
    async replaceGeneratedChapterFactMemories() {
      throw new Error('memory writer should not run after failed foreshadow alignment');
    },
  };
  const service = new FactExtractorService(prisma as never, llm as never, memoryWriter as never);

  await assert.rejects(
    () => service.extractChapterFacts('p1', 'c1', 'draft1'),
    /fact_extractor\.foreshadow_alignment 缺少候选伏笔判断/,
  );
  assert.equal(transactionCalled, false);
});

test('FactExtractorService rejects unresolved chapter-scope foreshadows in their recovery chapter', async () => {
  let transactionCalled = false;
  const existingForeshadow = {
    id: 'foreshadow-1',
    projectId: 'p1',
  ...
  const prisma = {
    ...
    chapterDraft: { async findFirst() { return { id: 'draft1', chapterId: 'c1', content: '旧玉佩只是轻轻发光，众人尚未明白它的作用。' }; } },
    foreshadowTrack: { async findMany() { return [existingForeshadow]; } },
    async $transaction() {
      transactionCalled = true;
      return {};
  ...
  const llm = {
    async chat() { return { text: '旧玉佩只是轻轻发光。' }; },
    async chatJson(_messages: unknown, options: { appStep: string }) {
      if (options.appStep === 'fact_extractor.foreshadow_alignment') {
        return { data: [{ foreshadowTrackId: 'foreshadow-1', status: 'planted', evidence: '旧玉佩只是轻轻发光，众人尚未明白它的作用。' }] };
      }
      return { data: [] };
    },
  };
...
test('FactExtractorService rejects new auto-extracted chapter-scope foreshadows that are not resolved', async () => {
  let transactionCalled = false;
  const prisma = {
    chapter: {
      async findFirst() {
  ...
  const llm = {
    async chat() { return { text: '旧玉佩只是轻轻发光。' }; },
    async chatJson(_messages: unknown, options: { appStep: string }) {
      if (options.appStep === 'fact_extractor.foreshadows') {
        return { data: [{ title: '旧玉佩异动', detail: '旧玉佩轻轻发光但用途未明', status: 'planted', scope: 'chapter' }] };
      }
      return { data: [] };
    },
  };
  const memoryWriter = {
    async replaceGeneratedChapterFactMemories() {
      throw new Error('memory writer should not run after invalid foreshadow extraction');
    },
  };
  const service = new FactExtractorService(prisma as never, llm as never, memoryWriter as never);
...
test('AppModule compiles with phase4 CRUD and phase5 quality modules registered', async () => {
  ...
  assert.equal(builtinSkill.defaultTools.includes('segment_chapter_outline_batches'), false);
  assert.equal(builtinSkill.defaultTools.includes('generate_chapter_outline_batch_preview'), false);
  assert.equal(builtinSkill.defaultTools.includes('merge_chapter_outline_batch_previews'), false);
  ...
  assert.match(volumeOutlineManifest.whenNotToUse.join(' | '), /重写.*章节细纲|章节细纲.*重写/);
  ...
  assert.match(storyUnitsManifest.whenNotToUse.join(' | '), /重写.*章节细纲|章节细纲.*重写/);
...
test('generate_outline_preview LLM timeout 直接抛错且不生成 fallback', async () => {
  ...
  assert.equal(progress[0].phase, 'calling_llm');
  assert.equal(progress[0].timeoutMs, DEFAULT_LLM_TIMEOUT_MS + 30_000);
...
test('generate_chapter_outline_preview 生成单章细纲并保留上一章接力卡', async () => {
  ...
  assert.equal(typeof receivedOptions?.onStreamProgress, 'function');
  assert.equal(receivedOptions?.maxTokens, 16000);
  assert.equal((receivedOptions?.jsonSchema as Record<string, unknown> | undefined)?.name, 'chapter_outline_preview');
  assert.doesNotMatch(receivedMessages[0].content, /"chapters"|chapters/);
  assert.match(receivedMessages[0].content, /章节设计检查/);
  assert.match(receivedMessages[0].content, /冲突升级/);
  assert.match(receivedMessages[1].content, /本章创作维度/);
  ...
  assert.match(receivedMessages[1].content, /UNIT_PREVIOUS_CHAPTER_ONE/);
  assert.match(receivedMessages[1].content, /UNIT_PREVIOUS_HOOK_ONE/);
  assert.match(receivedMessages[1].content, /第二章钩子/);
  assert.equal(llmUsages[0].model, 'mock-chapter-outline');
});

test('generate_chapter_outline_preview retries once when initial JSON is truncated', async () => {
  let mainCalls = 0;
  let retryMessages: Array<{ role: string; content: string }> = [];
  const llm = {
    async chatJson(messages: Array<{ role: string; content: string }>, options: Record<string, unknown>) {
      ...
      retryMessages = messages;
      return {
        data: { chapter: createOutlineChapter(1, 1), risks: [] },
        result: { model: 'mock-chapter-outline-retry', usage: { total_tokens: 91 }, rawPayloadSummary: { finishReason: 'stop' } },
      };
    },
  };
  const tool = new GenerateChapterOutlinePreviewTool(llm as never);
  ...
  const result = await tool.run(
    {
      context: { project: { title: 'json retry project' }, characters: [{ name: '\u6797\u6f88' }, { name: '\u6c88\u6816' }] },
      volumeOutline: {
        volumeNo: 1,
        title: 'json retry volume',
        synopsis: 'synopsis',
        objective: 'objective',
        chapterCount: 4,
        narrativePlan: createVccNarrativePlanForChapterCount(4),
      },
      instruction: 'generate chapter outline',
      volumeNo: 1,
      chapterNo: 1,
      chapterCount: 4,
    },
    { agentRunId: 'run-chapter-outline-json-retry', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
  );
...
test('generate_chapter_outline_preview quality review uses compact outline output', async () => {
  let qualityReviewPrompt = '';
  const narrativePlan = createVccNarrativePlanForChapterCount(4) as Record<string, unknown>;
  narrativePlan.qualityReviewSentinel = 'QUALITY_REVIEW_MUST_NOT_INCLUDE_FULL_NARRATIVE_PLAN';
  const llm = {
    async chatJson(messages: Array<{ role: string; content: string }>, options: Record<string, unknown>) {
      if (isChapterOutlineQualityReviewCall(options)) {
        qualityReviewPrompt = messages[1]?.content ?? '';
        return { data: createPassingChapterOutlineQualityReview(), result: { model: 'mock-chapter-outline-quality' } };
      }
      return {
        data: { chapter: createOutlineChapter(1, 1), risks: [] },
  ...
  await tool.run(
    {
      context: { project: { title: 'compact quality review' }, characters: [{ name: '\u6797\u6f88' }, { name: '\u6c88\u6816' }] },
      volumeOutline: { volumeNo: 1, title: 'volume', synopsis: 'synopsis', objective: 'objective', chapterCount: 4, narrativePlan },
      instruction: 'generate chapter outline',
      volumeNo: 1,
      chapterNo: 1,
      chapterCount: 4,
    },
    { agentRunId: 'run-chapter-outline-quality-compact', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
  );

  assert.match(qualityReviewPrompt, /"chapter"/);
  assert.match(qualityReviewPrompt, /Chapter design dimensions/);
  assert.doesNotMatch(qualityReviewPrompt, /QUALITY_REVIEW_MUST_NOT_INCLUDE_FULL_NARRATIVE_PLAN/);
  assert.doesNotMatch(qualityReviewPrompt, /<truncated>/);
});
...
test('generate_chapter_outline_preview craftBrief 局部缺字段可由 LLM 修复', async () => {
  ...
  assert.equal(calls[0].options.stream, true);
  assert.equal(calls[1].options.stream, true);
  assert.equal(calls[2].options.stream, true);
  assert.equal(calls[2].options.streamIdleTimeoutMs, DEFAULT_LLM_TIMEOUT_MS);
  assert.match(calls[1].messages[1].content, /irreversibleConsequence/);
...
test('COCF-P1 generate_chapter_outline_preview quality review fails once then regenerates', async () => {
  ...
  assert.equal(calls.every((call) => call.options.stream === true), true);
  assert.equal(calls.every((call) => call.options.streamIdleTimeoutMs === DEFAULT_LLM_TIMEOUT_MS), true);
  assert.equal(calls.every((call) => typeof call.options.onStreamProgress === 'function'), true);
