...
test('PersistWorldbuildingTool 只新增通过校验的新世界观条目并跳过同名设定', async () => {
  ...
  const tool = new PersistWorldbuildingTool(prisma as never);
  const result = await tool.run(
    {
      preview: {
        entries: [
...
test('PersistWorldbuildingTool 阻止未通过校验的世界观预览写入', async () => {
  const tool = new PersistWorldbuildingTool({} as never);
  await assert.rejects(
    () => tool.run(
      {
...
test('FactExtractorService 抽取事实后同步生成 pending_review 记忆', async () => {
  ...
  const prisma = {
    ...
    async $transaction(callback: (tx: unknown) => Promise<unknown>) {
      return callback({
        storyEvent: { async deleteMany() { return { count: 0 }; }, async create(args: { data: Record<string, unknown> }) { return { id: 'event1', ...args.data }; } },
        characterStateSnapshot: { async deleteMany() { return { count: 0 }; }, async create(args: { data: Record<string, unknown> }) { return { id: 'state1', ...args.data }; } },
        foreshadowTrack: { async deleteMany() { return { count: 0 }; }, async create(args: { data: Record<string, unknown> }) { return { id: 'foreshadow1', ...args.data }; } },
      });
    },
  };
  const llm = {
    ...
    async chatJson(_messages: unknown, options: { appStep: string }) {
      ...
      return { data: [{ title: '旧玉佩异动', detail: '旧玉佩在雨夜再次发光', status: 'planted' }] };
    },
  };
  const memoryWriter = {
    async replaceGeneratedChapterFactMemories(input: Record<string, unknown>) {
      ...
      return {
        deletedCount: 0,
        createdCount: 4,
        embeddingAttachedCount: 4,
        chunks: [
          { id: 'm-summary', memoryType: 'summary', summary: '摘要', status: 'auto' },
          { id: 'm-event', memoryType: 'event', summary: '事件', status: 'auto' },
          { id: 'm-state', memoryType: 'character_state', summary: '状态', status: 'pending_review' },
          { id: 'm-foreshadow', memoryType: 'foreshadow', summary: '伏笔', status: 'pending_review' },
        ],
      };
    },
  };
  ...
  assert.equal(result.createdMemoryChunks, 4);
  assert.equal(result.pendingReviewMemoryChunks, 2);
});
