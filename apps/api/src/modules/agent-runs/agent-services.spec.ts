...
test('VCC volume outline preview rejects unknown existing character and prompts candidate use', async () => {
  ...
  assert.match(receivedMessages[0].content, /不在白名单中的人物/);
  assert.match(receivedMessages[1].content, /既有角色白名单/);
  assert.match(receivedMessages[1].content, /newCharacterCandidates/);
});
      ...
      return {
        data: {
          lorebookEntries: [
            ...
            { title: '', content: 'should fallback title' },
          ],
          characters: [{ name: '不应出现' }],
          writingRules: [{ title: '不应出现' }],
          chapters: [{ title: '不应出现' }],
...
test('Planner 将只重写卷大纲的旧聚合计划改写为卷纲专用链路', () => {
  ...
  assert.deepEqual(plan.steps.map((step) => step.tool), ['inspect_project_context', 'generate_volume_outline_preview', 'persist_volume_outline', 'persist_volume_character_candidates']);
  ...
  assert.deepEqual(plan.steps[3].args, { preview: '{{steps.2.output}}', approveAll: true, includeRelationshipArcs: true });
  assert.deepEqual(plan.steps[3].runIf, { ref: '{{steps.2.output.volume.narrativePlan.characterPlan.newCharacterCandidates.length}}', operator: 'gt', value: 0 });
...
test('Planner 在单元故事链路中追加卷级候选人物写入并保证写入顺序', async () => {
  const toolList = [
    createTool({ name: 'inspect_project_context', requiresApproval: false, riskLevel: 'low', sideEffects: [] }),
    createTool({ name: 'generate_volume_outline_preview', requiresApproval: false, riskLevel: 'low', sideEffects: [] }),
    createTool({ name: 'generate_story_units_preview', requiresApproval: false, riskLevel: 'low', sideEffects: [] }),
...
test('Planner 将残缺单章细纲计划展开为所有章节 Tool 调用', () => {
  ...
  assert.equal(plan.steps[8].requiresApproval, true);
  assert.equal(plan.steps[9].tool, 'persist_volume_character_candidates');
  assert.deepEqual(plan.steps[9].args, { preview: '{{steps.7.output}}', approveAll: true, includeRelationshipArcs: true });
  assert.deepEqual(plan.steps[9].runIf, { ref: '{{steps.7.output.volume.narrativePlan.characterPlan.newCharacterCandidates.length}}', operator: 'gt', value: 0 });
});
