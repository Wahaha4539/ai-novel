              ...
              storyUnits: [
                {
                  ...
                  localGoal: '重写逃生桥压力链',
                  localConflict: '旧桥罪名与潮汛风险同时压迫工队',
                  serviceFunctions: ['mainline', 'relationship_shift', 'foreshadow'],
                  payoff: '工队拿到逃生桥试压结果',
                  stateChangeAfterUnit: '主角从被动背罪转为掌握桥况证据',
                },
              ],
            }),
          },
...
test('generate_chapter_outline_preview 生成单章细纲并保留上一章接力卡', async () => {
  ...
  const result = await tool.run(
    {
      context: { project: { title: '逆潮脊梁' }, volumes: [{ volumeNo: 1, title: '罪桥初潮', synopsis: '小归潮逃生', objective: '修成逃生桥' }], characters: [{ name: '林澈' }] },
      volumeOutline: {
        volumeNo: 1,
        title: '罪桥初潮',
        synopsis: '小归潮逃生',
...
test('generate_outline_preview 为 60 章逐章请求 LLM 并传递章节接力卡', async () => {
  ...
  const result = await tool.run(
    { instruction: '为第 1 卷生成 60 章细纲', volumeNo: 1, chapterCount: 60, context: { characters: [{ name: '林澈' }] } },
    {
      agentRunId: 'run1',
      projectId: 'p1',
      mode: 'plan',
...
test('generate_outline_preview 保留 LLM craftBrief', async () => {
  ...
  const tool = new GenerateOutlinePreviewTool(llm as never);
  const result = await tool.run(
    { instruction: '生成第二卷细纲', volumeNo: 2, chapterCount: 2, context: { characters: [{ name: '林澈' }] } },
    { agentRunId: 'run1', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
  );
...
test('generate_outline_preview 缺失 storyUnit 字段时直接报错', async () => {
  ...
  await assert.rejects(
    () => tool.run(
      { instruction: '生成第二卷细纲', volumeNo: 2, chapterCount: 1, context: { characters: [{ name: '林澈' }] } },
      { agentRunId: 'run1', projectId: 'p1', mode: 'plan', approved: false, outputs: {}, policy: {} },
    ),
    /storyUnit/,
  );
