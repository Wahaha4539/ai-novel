    ...
    const messages = [
      {
        ...
        content: [
          ...
          '可引用上下文：{{context.session.currentProjectId}}、{{context.session.currentChapterId}}、{{context.session.currentDraftId}}、{{context.session.currentDraftVersion}}、{{context.session.selectedText}}、{{context.session.selectedRange}}、{{context.session.selectedParagraphRange}}、{{context.project.defaultWordCount}}、{{context.attachments.0}}、{{context.attachments.0.url}}、{{context.session.guided.currentStep}}、{{context.session.guided.currentStepData}}。',
        ...
        content: JSON.stringify(
          {
            ...
            taskTypeGuidance: {
              ...
              chapter_passage_revision: '当上下文来自 editor_passage_agent，且有 selectedText、selectedRange、currentDraftId、currentDraftVersion 时，使用 revise_chapter_passage_preview -> apply_chapter_passage_revision，只修改当前选区。可处理选区润色、压缩节奏、调整角色口吻、降低解释感、保留事实重写表达。不要误用 polish_chapter 或 rewrite_chapter；只有用户明确要求整章润色/整章重写时，才改走整章工具。若 selectedRange 或 currentDraftId 缺失，不要调用 passage 工具。',
