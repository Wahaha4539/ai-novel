    ...
    const messages = [
      {
        ...
        content: [
          ...
          '可引用上下文：{{context.session.currentProjectId}}、{{context.session.currentChapterId}}、{{context.session.currentDraftId}}、{{context.session.currentDraftVersion}}、{{context.session.selectedText}}、{{context.session.selectedRange}}、{{context.session.selectedParagraphRange}}、{{context.session.passageRevision.previewId}}、{{context.session.passageRevision.previousReplacementText}}、{{context.session.passageRevision.previousEditSummary}}、{{context.project.defaultWordCount}}、{{context.attachments.0}}、{{context.attachments.0.url}}、{{context.session.guided.currentStep}}、{{context.session.guided.currentStepData}}。',
          '当 sourcePage=editor_passage_agent 且 selectionIntent=chapter_passage_revision，同时具备 selectedText、selectedRange、currentDraftId、currentDraftVersion 时，taskType 必须优先选择 chapter_passage_revision，并使用 revise_chapter_passage_preview -> apply_chapter_passage_revision 的局部链路。',
          '当 editor_passage_agent 的上下文里存在 passageRevision.previewId 或 previousReplacementText，说明用户正在对上一版局部修订预览继续反馈；应继续沿用同一 selectedRange / selectedText / draftId，并把这些字段传给 revise_chapter_passage_preview.context.previousPreview，支持“保留第二句，其他重写”这类基于上一版预览的细化反馈。',
          'editor_passage_agent 选区任务默认只改选区，不得误规划为整章 rewrite_chapter 或整章 polish_chapter；只有用户明确说“整章润色 / 整章重写 / 全章重写 / 不沿用旧稿重写整章”时，才转向整章工具。',
          '如果 editor_passage_agent 缺少 selectedRange、selectedText、currentDraftId 或 currentDraftVersion，不要编造选区参数；改用非 passage 的 revision 路线，或在 missingInfo 中要求重新选择。',
          ...
          '用户只要求“卷大纲 / 第 N 卷的大纲 / 重写某卷大纲”，且没有明确说单元故事、章节细纲、拆成 N 章、章节规划或 Chapter.craftBrief 时，选择 outline_design，但只编排 inspect_project_context -> generate_volume_outline_preview -> persist_volume_outline；卷纲必须一并生成/刷新 characterPlan.newCharacterCandidates 作为重要新增角色候选，但不要自动追加 persist_volume_character_candidates；不要生成章节细纲。',
          '用户明确说“增加角色 / 扩充角色 / 角色太少 / 角色不够 / 撑不起整本书 / 扩充角色阵容”等角色扩充目标时，选择 outline_design；如果没有同时要求章节细纲，则用卷纲链路生成/刷新卷级 characterPlan.newCharacterCandidates；如果同时要求章节细纲，则先 generate_volume_outline_preview 刷新角色候选和卷级规划，再 generate_story_units_preview，随后逐章 generate_chapter_outline_preview 并把 volumeOutline/storyUnitPlan 传入每个单章步骤。',
          '用户要求“单元故事 / 支线故事 / 人物登场 / 人物情感 / 背景故事 / 丰富单元分类”时选择 outline_design，并编排 inspect_project_context -> generate_story_units_preview -> persist_story_units；只有用户明确要求同时重写/更新卷大纲时，才额外加入 persist_volume_outline；persist_volume_character_candidates 仍只在用户明确要求保存全部或选定候选角色时使用。',
          '用户要求“章节细纲 / 第 N 卷章节细纲 / 章节规划 / 重写章节细纲 / 重新生成章节细纲”，且没有明确说重写卷纲、重新拆成 N 章、改变章数或从头规划时，选择 outline_design，并优先承接 inspect_project_context 中已有 Volume.narrativePlan/storyUnitPlan；无论 chapterCount 多大，都使用 generate_chapter_outline_preview（每章一个步骤）-> merge_chapter_outline_previews -> persist_outline；不要追加末尾 validate_outline，不要重新生成卷大纲或单元故事。',
          'HIGHER PRIORITY chapter-outline scope rule: “重写第一卷的章节细纲 / 重写章节细纲 / 重新生成章节细纲” means replace chapter outlines only. It does not mean “重写卷纲”, “重新拆分章节结构”, “重写单元故事”, or “改变章数”.',
          '用户明确要求“重写卷纲 / 重写卷细纲 / 从头规划卷细纲 / 重新拆成 N 章 / 改变章节数 / 重新规划章数 / 重新划分单元故事”时选择 outline_design，并先重建 generate_volume_outline_preview -> generate_story_units_preview；然后逐章使用 generate_chapter_outline_preview（每章一个步骤）-> merge_chapter_outline_previews -> persist_outline；若完整 outline_preview 中含 newCharacterCandidates 且工具可用，也不要自动追加 persist_volume_character_candidates，除非用户明确要求保存候选角色；不要追加末尾 validate_outline，不要误判为 write_chapter。',
          ...
          'CURRENT chapter-outline rule: use the single-chapter chain for long chapter outlines too. Do not use segment_chapter_outline_batches, generate_chapter_outline_batch_preview, or merge_chapter_outline_batch_previews in planner-generated chapter-outline plans; dense craftBrief JSON must stay one chapter per LLM call.',
          'HIGHER PRIORITY character-candidate rule: do not add persist_volume_character_candidates to ordinary volume-outline or chapter-outline plans. A merged outline preview may carry volume.characterPlan.newCharacterCandidates inside narrativePlan; writing those candidates to the official Character table is a separate explicit approval task. Only call persist_volume_character_candidates if the user explicitly asks to save all or selected volume candidates, and then args must include approveAll=true or approvedCandidateIds/approvedCandidateNames.',
          'For single-chapter outline steps, chapter 2 and later should pass previousChapter from the immediately preceding generate_chapter_outline_preview output. The tool also collects earlier generated chapters from the same storyUnit via ToolContext.outputs and includes them in the prompt as previousUnitChapters, so do not try to collapse multiple chapters into one LLM call.',
          'If target chapterCount=N differs from the target volume chapterCount in inspect_project_context, first rebuild upstream planning: generate_volume_outline_preview(chapterCount=N) -> generate_story_units_preview(volumeOutline={{steps.X.output.volume}}, chapterCount=N), then pass that same volumeOutline and storyUnitPlan into every generate_chapter_outline_preview step and merge_chapter_outline_previews. This is a deterministic plan consistency requirement, not a creative-content judgment.',
        ...
        content: JSON.stringify(
          {
            ...
            taskTypeGuidance: {
              ...
              chapter_passage_revision: '当上下文来自 editor_passage_agent，且有 selectedText、selectedRange、currentDraftId、currentDraftVersion 时，使用 revise_chapter_passage_preview -> apply_chapter_passage_revision，只修改当前选区。可处理选区润色、压缩节奏、调整角色口吻、降低解释感、保留事实重写表达。若 context.session.passageRevision.previewId / previousReplacementText 存在，说明这是基于上一版 preview 的连续反馈；应继续沿用同一选区，并把 previousPreview 透传给 revise_chapter_passage_preview.context.previousPreview，支持“保留第二句，其他重写”等指令。不要误用 polish_chapter 或 rewrite_chapter；只有用户明确要求整章润色/整章重写时，才改走整章工具。若 selectedRange 或 currentDraftId 缺失，不要调用 passage 工具。',
              outline_design: '设计大纲。若用户只说卷大纲/第N卷的大纲/重写某卷大纲，且未要求单元故事、章节细纲、拆成N章、章节规划或 Chapter.craftBrief，只使用 generate_volume_outline_preview，审批写入使用 persist_volume_outline，不要生成章节细纲；卷纲必须一并生成/刷新 characterPlan.newCharacterCandidates 作为重要新增角色候选，但只有用户明确要求保存候选角色时才追加 persist_volume_character_candidates。若用户明确说增加角色、扩充角色、角色太少、角色不够、撑不起整本书或扩充角色阵容，应把它视为卷级角色规划任务：没有章节细纲目标时走卷纲链路；同时有章节细纲目标时，先 generate_volume_outline_preview 刷新角色候选，再 generate_story_units_preview 分配角色功能，然后逐章 generate_chapter_outline_preview 并把上游 volumeOutline/storyUnitPlan 传入每章。若用户要求单元故事、支线故事、人物登场、人物情感、背景故事或丰富单元分类，应使用 generate_story_units_preview，审批写入只使用 persist_story_units；generate_volume_outline_preview 若只是给单元故事提供上游卷纲参考，不要写入 persist_volume_outline，除非用户明确要求同时更新卷大纲。若用户只要求章节细纲/第N卷章节细纲/章节规划/重写章节细纲/重新生成章节细纲，且没有明确要求重写卷纲、重新拆成N章、改变章数、从头规划或扩充角色，应承接 inspect_project_context 中已有 Volume.narrativePlan/storyUnitPlan，直接使用 generate_chapter_outline_preview 为每一章生成可见 Tool 调用，再 merge_chapter_outline_previews；不要重新生成卷大纲或单元故事。“重写第一卷的章节细纲”只替换章节细纲，不代表重建卷纲或单元故事。若用户明确要求重写卷纲、重写卷细纲、从头规划卷细纲、重新拆成 N 章、改变章节数、重新规划章数、重新划分单元故事或扩充角色，应先使用 generate_volume_outline_preview 生成卷大纲，再用 generate_story_units_preview 生成单元故事计划，然后逐章生成；审批写入使用 persist_outline；不要误判为写正文。',
              outline_chapter_batching: 'For long chapter-outline work (chapterCount > 12, especially 60 chapters), do not use segmented batch tools. Ordinary chapter-outline rewrites use inspect_project_context -> one visible generate_chapter_outline_preview step per chapter -> merge_chapter_outline_previews -> persist_outline, reusing the target volume narrativePlan/storyUnitPlan from context. Only insert generate_volume_outline_preview/generate_story_units_preview when the user explicitly asks to rebuild volume planning, expand/add roles, re-split the volume, change chapterCount, or plan from scratch. When role expansion is part of a chapter-outline request, generate or refresh volumeOutline.characterPlan.newCharacterCandidates first, then pass that same volumeOutline and the refreshed storyUnitPlan into every single-chapter preview so chapters can cite source=volume_candidate. Do not append terminal validate_outline. Do not add persist_volume_character_candidates to ordinary chapter-outline plans; saving volume character candidates to the official Character table is separate and requires explicit approveAll=true or approvedCandidateIds/approvedCandidateNames. For ordinary chapter-outline requests, default to the target agentContext.volumes[].chapterCount and storyUnitPlan. If the user explicitly changes chapterCount so it differs from agentContext.volumes[].chapterCount, rebuild volumeOutline and storyUnitPlan first, then pass volumeOutline and storyUnitPlan into every single-chapter preview step and merge. Each single-chapter prompt automatically includes earlier generated outlines from the same storyUnit through ToolContext.outputs, while previousChapter should still reference the immediately preceding chapter.',
              project_import_preview: '拆解导入文案，并按用户指定目标产物生成预览。只要大纲时不要生成角色/世界观/写作规则；要求全套时才生成项目资料、剧情大纲、角色、世界观和写作规则。',
              chapter_revision: '修改当前章或已有章节草稿、增强节奏/压迫感、保留结局等禁改约束；若上下文来自 editor_passage_agent 且选区信息完整，应改用 chapter_passage_revision；若用户要求重写或不沿用旧稿，使用 rewrite_chapter。',
      ...
      [
        {
          ...
          content: [
            ...
            '修复卷大纲计划时，如果用户没有明确要求单元故事、章节细纲、拆成 N 章、章节规划或 Chapter.craftBrief，taskType 应为 outline_design，并使用 generate_volume_outline_preview -> persist_volume_outline；卷纲预览要保留/刷新 characterPlan.newCharacterCandidates，但只有用户明确要求保存全部或选定候选角色时才追加 persist_volume_character_candidates；不要生成章节细纲。',
            '修复角色扩充计划时，如果用户明确说增加角色、扩充角色、角色太少、角色不够、撑不起整本书或扩充角色阵容，应把新增重要角色放入卷级 characterPlan.newCharacterCandidates；如果同时要求章节细纲，先 generate_volume_outline_preview，再 generate_story_units_preview，然后逐章 generate_chapter_outline_preview，并把 volumeOutline/storyUnitPlan 传给每个单章步骤。',
            ...
            '修复章节细纲计划时，如果用户没有明确要求重写卷纲、重新拆成 N 章、改变章数或从头规划，应承接 inspect_project_context 中已有 Volume.narrativePlan/storyUnitPlan，直接用 generate_chapter_outline_preview 为每章生成独立步骤，再用 merge_chapter_outline_previews 合并；不要重新生成卷大纲或单元故事。“重写章节细纲”仍属于这一类。',
            '修复明确重写卷纲、重写卷细纲、从头规划卷细纲、重新拆成 N 章、改变章节数、重新规划章数或重新划分单元故事的计划时，taskType 应为 outline_design，并使用 generate_volume_outline_preview 生成卷大纲，再用 generate_story_units_preview 生成单元故事计划，再用 generate_chapter_outline_preview 为每章生成独立步骤，最后用 merge_chapter_outline_previews 合并；只有用户明确要求保存候选角色时才追加 persist_volume_character_candidates；只有写正文/生成正文才使用 chapter_write，明确重写/不沿用旧稿时使用 rewrite_chapter，拆成场景/SceneCard 才使用 scene_card_planning。',
            ...
            'CURRENT repair rule: even when chapterCount > 12, repair chapter-outline plans to the single-chapter shape: one generate_chapter_outline_preview step per chapter, previousChapter referencing the immediately preceding chapter, merge_chapter_outline_previews.previews covering every chapter output in order, and approval-gated persist_outline. Do not append terminal validate_outline.',
            'HIGHER PRIORITY repair rule: remove persist_volume_character_candidates unless the user explicitly asked to save all or selected volume candidates; if you keep that tool, its args must include approveAll=true or approvedCandidateIds/approvedCandidateNames.',
            'If validationError says target chapterCount differs from context volume chapterCount, repair by inserting generate_volume_outline_preview(chapterCount=N), generate_story_units_preview(volumeOutline={{steps.X.output.volume}}, chapterCount=N), and by passing volumeOutline/storyUnitPlan to every generate_chapter_outline_preview step and merge_chapter_outline_previews.',
