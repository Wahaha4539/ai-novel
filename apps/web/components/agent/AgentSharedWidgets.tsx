...
const AGENT_TOOL_UI_EXPLANATIONS: Record<string, AgentToolUiExplanation> = {
  ...
  segment_chapter_outline_batches: {
    label: '切分章节细纲批次',
    purpose: '按 storyUnitPlan.chapterAllocation 或连续章节段切分整卷章节细纲范围，并校验覆盖连续、不重叠、不越界。',
    output: '章节批次范围与覆盖报告',
    frontendSurface: 'Agent 计划 / 章节批次覆盖列表',
    artifactTypes: ['chapter_outline_batch_plan'],
    usesLlm: false,
  },
  generate_chapter_outline_batch_preview: {
    label: '批次生成章节细纲',
    purpose: '一次生成连续章节段的章节细纲、完整 craftBrief 和 characterExecution，并通过严格结构校验。',
    output: '批次章节细纲预览',
    frontendSurface: 'Agent 计划与产物预览 / 批次章节细纲摘要',
    artifactTypes: ['chapter_outline_batch_preview'],
    usesLlm: true,
  },
  ...
  merge_chapter_outline_batch_previews: {
    label: '合并批次章节细纲',
    purpose: '合并多个批次预览为标准 outline_preview，并校验章节覆盖完整、无缺章、无重复、无越界。',
    output: '完整大纲预览',
    frontendSurface: 'Agent 产物预览 / 大纲校验与写入前输入',
    artifactTypes: ['outline_preview'],
    usesLlm: false,
