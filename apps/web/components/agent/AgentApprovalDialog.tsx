    ...
    <div className="panel p-5 flex flex-wrap items-center justify-between gap-4" style={{ borderColor: canAct ? 'rgba(34,197,94,0.35)' : 'var(--border-dim)' }}>
      <div>
        ...
        {riskSummary.length > 0 && (
          <ul className="mt-3 grid gap-1 text-[11px] leading-5" style={{ color: '#fbbf24' }}>
            {riskSummary.map((item) => <li key={item}>• {item}</li>)}
          </ul>
        )}
      </div>
      <div className="flex gap-3">
        {hasCurrentRun && <button onClick={() => void onCancel()} disabled={loading || terminal} className="px-4 py-3 text-sm" style={{ borderRadius: '0.8rem', border: '1px solid var(--status-err)', color: 'var(--status-err)', background: 'transparent', opacity: terminal ? 0.5 : 1 }}>取消</button>}
