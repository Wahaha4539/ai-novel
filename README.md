...
> 当前核心功能 **不需要启动 Python Worker**，Python Worker 源码已从仓库删除。
...
不需要。当前章节生成、润色、事实抽取、校验、记忆重建、MemoryWriter、Embedding/pgvector 召回和 Agent 执行都在 `apps/api` 内完成，Python Worker 源码已从仓库删除。
...
- 如需查看旧 Python pipeline，请通过 git 历史追溯删除前的实现。
